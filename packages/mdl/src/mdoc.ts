...
import { MDocStatus } from '@m-doc/types';
...
function parseRawDeviceSigned(
  ...
  Object.entries(nameSpaces.data).forEach(([name, claims]) => {
...
function parseRawNameSpaces(raw: RawDoc['issuerSigned']['nameSpaces']) {
  const nameSpaces: Map<string, Array<IssuerSignedItem>> = new Map();
  Object.entries(raw).forEach(([name, items]) => {
    const issuerSignedItems = items.map((item) => new IssuerSignedItem(item));
